...
nhn.husky.SE2M_AttachQuickPhoto = jindo.$Class({		
	...
	$ON_MSG_APP_READY : function(){
	    if(navigator.userAgent.search('Android|iPhone') <= 0) {
	        this.oApp.exec("REGISTER_UI_EVENT", ["photo_attach", "click", "ATTACHPHOTO_OPEN_WINDOW"]);
	    }
	    
	    // mobile upload
	    else {
	        this.oApp.exec("REGISTER_UI_EVENT", ["photo_attach", "click", "TRIGGER_FILE_DIALOG"]);
	        
	        var self = this;
	        
	        $Element('attach-image-trigger').delegate('click', '.se2_photo', function(e) {
	            $('attach-image').click();
	        });	    
	        $Element('attach-image-trigger').delegate('change', '#attach-image', function(e) {
	            parent.hi.ImageUpload.ajaxUpload(document.getElementById('attach-image'))
	            .done(function(d) {
	                var fileUrlSplit = d.data.fileRelUrl.split('/');
	                
	                var oFileInfo = {
	                        bNewLine: true,
	                        sFileName: fileUrlSplit[fileUrlSplit.length - 1],
	                        sFileURL: d.data.fileRelUrl
	                };
	                
	                self.$ON_SET_PHOTO([oFileInfo]);
	            });	    
	        });
	    }
	},
